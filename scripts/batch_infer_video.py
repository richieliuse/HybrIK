...
def parse_args():
    ...
    #                     default='',
    #                     type=str)
    parser.add_argument("--input-path", help="video name", default="", type=str)
    parser.add_argument("--output-path", help="output folder", default="", type=str)
    # parser.add_argument(
    #     "--save-pk",
    #     default=False,
    #     dest="save_pk",
...
def load_hybrik_model(cfg, ckpt):
...
def process_video(
    ...
    os.system(f"ffmpeg -i {video_path} {out_dir}/raw_images/{video_basename}-%06d.png")
    ...
    for img_path in tqdm(img_path_list):
        ...
        with torch.no_grad():
            ...
            if save_pose:
                ...
                pred_theta = (
                    pose_output.pred_theta_mats.squeeze(dim=0).cpu().data.numpy()
                )
    ...
    if save_pose:
        ...
        with open(os.path.join(out_dir, f"{video_basename}.pkl"), "wb") as fid:
            pk.dump(res_db, fid)
...
def process_videos(
    input_path, output_path, hybrik_model, det_model, transformation, gpu=0
):
    # 确保输出目录存在
    ...
    for root, _, files in os.walk(input_path):
        for file in files:
            if file.endswith(
                ...
                print(f"Processed {video_path} and saved results to {save_dir}")
...
def main():
    ...
    hybrik_model = load_hybrik_model(cfg, CKPT)
